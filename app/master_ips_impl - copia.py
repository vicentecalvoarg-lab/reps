"""
MASTER IPS - Página para filtrar la base maestra de IPS, generar tabla de sedes únicas,
mostrar treemap de giros y calcular oferta por SERVICIO|ESPECIALIDAD.

Esta versión incluye:
 - Resumen single-line con los filtros aplicados encima de la tabla de oferta.
 - Tabla ordenada por indicador (mayor a menor).
 - Selector desplegable para elegir una combinación SERVICIO|ESPECIALIDAD y
   mostrar debajo la lista de sedes que la ofrecen con la cantidad ofertada por sede.
 - En la tabla detalle por sede se muestra OFERTA POR SEDE y GIROS (MM COP).
 - Gráfico circular (pie) con % y número de servicios por sede para la combinación seleccionada.
 - Integración con C:/REPS/data/sugeridos.csv para clasificar el indicador como Alto/Normal/Bajo.
 - Diagnóstico interactivo (ligero) para mapear manualmente combos no emparejados con sugeridos.csv
...
def _normalize_name_for_match(s: Optional[str]) -> str:
    """Upper, strip, remove accents and normalize spaces for matching."""
    if pd.isna(s) or s is None:
        return ""
    s = str(s).strip().upper()
...
# ---------------- Services per scale (composite) - final version ----------------
...
def render_services_per_scale_composite(df_source: pd.DataFrame, df_censo_path: str,
    ...
    También incluye diagnóstico interactivo para mapear combos no emparejados.
    """
    ...
    if svc_col is None and spec_col is None:
        st.info("No hay columna de servicio o especialidad detectada para calcular oferta/populación.")
        return
    ...
    # Ensure numeric TOTAL_OFERTA_NUM exists for sorting/exports
    if 'TOTAL_OFERTA_NUM' not in display_df.columns:
        try:
    ...
    # --- DIAGNÓSTICO LIGERO Y MAPEADO MANUAL (versión segura) ---
    with st.expander("Diagnóstico sugeridos.csv (abrir para ver y mapear)"):
        st.write("Resumen del archivo sugeridos.csv y emparejado automático (ligero).")
        if sugeridos_df is None or sugeridos_df.empty:
            st.warning(f"No se encontró {sugeridos_path} o está vacío; no se podrá clasificar con benchmarks.")
            sugeridos_df = pd.DataFrame()
            sug_keys = []
        else:
            st.write("Columnas detectadas en sugeridos.csv:", sugeridos_df.columns.tolist())
            st.dataframe(sugeridos_df.head(5))

            # Construir clave SERVICE_U preferida
            ...
            svc_col_alt = _find_column_like(sugeridos_df, ["Servicio", "Servico", "Servicio REPS"])
            spec_col_alt = _find_column_like(sugeridos_df, ["Especialidad", "Especialidad REPS"])
            if svc_col_sug:
                ...
                sugeridos_df['SERVICE_U'] = sugeridos_df.index.astype(str)
            ...
            # matching automático ligero: exacto y contains (fuzzy sólo para el seleccionado)
            with st.spinner("Realizando emparejado automático (ligero)..."):
                def _match_light(combo):
                    k = _normalize_name_for_match(combo)
                    if k in sug_keys:
                        return k, "exact"
                    for s in sug_keys:
                ...
                display_df['SUG_KEY_MATCH'] = display_df['SERVICIO | ESPECIALIDAD'].astype(str).map(lambda x: _match_light(x)[0])
                display_df['SUG_MATCH_METHOD'] = display_df['SERVICIO | ESPECIALIDAD'].astype(str).map(lambda x: _match_light(x)[1])

            n_total = len(display_df)
            n_matched = int(display_df['SUG_KEY_MATCH'].notnull().sum()) if 'SUG_KEY_MATCH' in display_df.columns else 0
            st.info(f"Benchmark matching automático: {n_matched:,} / {n_total:,} combos emparejados (exact/contains).")

            # Mostrar primeros no emparejados y permitir mapeo manual de uno a la vez
            ...
            if len(no_match) > 0:
                selected_unmatched = st.selectbox("Selecciona un combo no emparejado para mapear (uno a la vez)", options=["(ninguno)"] + no_match)
                if selected_unmatched and selected_unmatched != "(ninguno)":
                    k_norm = _normalize_name_for_match(selected_unmatched)
                    fuzzy_suggestions = difflib.get_close_matches(k_norm, sug_keys, n=8, cutoff=0.5) if len(sug_keys) > 0 else []
                    opts = ["(ninguno)"] + fuzzy_suggestions
                    if len(opts) == 1 and len(sug_keys) <= 50:
                        opts = ["(ninguno)"] + sug_keys
                    choose = st.selectbox(f"Asignar benchmark (para '{selected_unmatched}')", options=opts, index=0)
                    if choose and choose != "(ninguno)":
                        norm_choose = choose if choose in sug_keys else _normalize_name_for_match(choose)
                        display_df.loc[display_df['SERVICIO | ESPECIALIDAD'] == selected_unmatched, 'SUG_KEY_MATCH'] = norm_choose
                        display_df.loc[display_df['SERVICIO | ESPECIALIDAD'] == selected_unmatched, 'SUG_MATCH_METHOD'] = 'manual'
                        st.success(f"Asignado '{choose}' a '{selected_unmatched}'.")
                st.dataframe(pd.DataFrame({"sin_match_sample": no_match[:200]}))

    # classification function using the found key
    def _classify_using_sugkey(combo_label: str, indicator_value: float, sug_key: Optional[str]) -> str:
        ...
        if rec_row.empty:
            return "No benchmark"
        rec = rec_row.iloc[0]
        min_reco = rec.get('MIN_RECO', np.nan) if 'MIN_RECO' in rec else np.nan
        umbral = rec.get('UMBRAL_ALTO', np.nan) if 'UMBRAL_ALTO' in rec else np.nan
        try:
    ...
    # compute formatted indicator column and initial evaluation (may be updated by manual mappings)
    ...
    display_df['EVALUACIÓN (sugerido)'] = display_df.apply(lambda r: _classify_using_sugkey(r['SERVICIO | ESPECIALIDAD'], r['INDICADOR_POR_ESCALA'], r.get('SUG_KEY_MATCH')), axis=1)
    ...
    # Add reference text below the table as requested
    st.markdown("")
    st.markdown("La estimacion de Bajo, Normal o Alto surge de comparar los indicadores con tablas que proporcionan los benchmarks sugeridos basados en estándares de WHO, PAHO y literatura para LMICs")
        ...
        if combo_options:
            ...
            if selected_combo and selected_combo != "(ninguno)":
                ...
                if not selected_rows.empty:
                    ...
                    # ---------------- Pie chart debajo de la tabla detalle ----------------
                    try:
                        if 'NOMBRE SEDE' in disp_show.columns:
                            name_col = 'NOMBRE SEDE'
                        elif 'NIT' in disp_show.columns:
                            name_col = 'NIT'
                        else:
                            name_col = '_sede_key' if '_sede_key' in disp_show.columns else disp_show.columns[0]

                        if 'OFERTA POR SEDE' in disp_show.columns:
                            pie_df = disp_show[[name_col, 'OFERTA POR SEDE']].copy()
                            pie_df = pie_df.groupby(name_col, as_index=False)['OFERTA POR SEDE'].sum()
                            total_offerta = pie_df['OFERTA POR SEDE'].sum()
                            if total_offerta > 0:
                                svc_title = selected_combo
                                dept_info = applied_filters.get('Departamento','') if applied_filters else ""
                                mun_info = applied_filters.get('Municipio','') if applied_filters else ""
                                location_part = f"{dept_info} / {mun_info}" if dept_info and mun_info else (dept_info or mun_info or "")
                                title = f"{svc_title} — {location_part}" if location_part else f"{svc_title}"

                                fig = px.pie(pie_df, names=name_col, values='OFERTA POR SEDE', hole=0.35,
                                             title=title,
                                             labels={name_col: "", 'OFERTA POR SEDE': "Cantidad"})
                                fig.update_traces(textinfo='percent+value', hovertemplate='%{label}<br>%(value)s servicios<br>%{percent}')
                                fig.update_layout(margin=dict(t=60, l=10, r=10, b=10))
                                st.plotly_chart(fig, use_container_width=True)
                            else:
                                st.info("No hay oferta (OFERTA POR SEDE = 0) para graficar el detalle.")
                        else:
                            st.info("No hay columna 'OFERTA POR SEDE' para construir el gráfico circular.")
                    except Exception as e:
                        st.error(f"Error generando gráfico circular: {e}")

                    # debug: show aggregated giro rows for NITs (optional)
                    if st.checkbox("Mostrar comprobación: filas de giro agregadas (debug)", value=False):
                        nit_keys = disp['NIT_CLEAN'].unique().tolist() if 'NIT_CLEAN' in disp.columns else []
                        if nit_keys and 'agg_giro' in locals():
                            debug_rows = agg_giro[agg_giro['nit_normalized'].astype(str).map(lambda x: _clean_nit_str(x) or "").isin(nit_keys)]
                            st.write("Filas de giros agregadas para NITs en la tabla detalle:")
                            st.dataframe(debug_rows[['nit_normalized', '_amount', 'GIROS_MM_INT']].rename(columns={'_amount': 'TOTAL_VALOR'}), use_container_width=True)
                        else:
                            st.write("No hay datos de giros para mostrar en debug.")
                else:
                    st.info("No se encontraron sedes para la combinación seleccionada.")
    except Exception as e:
        st.error(f"Error preparando selector de sedes: {e}")

    # export oferta table
    buf = io.BytesIO()
    if st.button("Exportar tabla oferta (Excel)"):
        try:
            export = display_df.copy()
            if 'TOTAL_OFERTA_NUM' not in export.columns:
                export['TOTAL_OFERTA_NUM'] = pd.to_numeric(export.get('TOTAL OFERTA', export.get('TOTAL_OFERTA', export.get('SEDES (n)', 0))), errors='coerce').fillna(0.0)
            export = export.rename(columns={'SERVICIO | ESPECIALIDAD': 'SERVICIO | ESPECIALIDAD', 'SEDES (n)': 'SEDES (n)', 'TOTAL_OFERTA_NUM': 'TOTAL OFERTA', 'POBLACION': 'POBLACION (hab)', 'EVALUACIÓN (sugerido)': 'EVALUACIÓN (sugerido)'})
            export['POBLACION (hab)'] = export['POBLACION (hab)'] if 'POBLACION (hab)' in export.columns else export.get('POBLACION', 0)
            with pd.ExcelWriter(buf, engine="openpyxl") as writer:
                export.to_excel(writer, index=False, sheet_name="oferta_por_escala")
            buf.seek(0)
            st.download_button("Descargar Excel oferta_por_escala.xlsx", data=buf, file_name="oferta_por_escala.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        except Exception as e:
...
def run(filters: Dict[str, Any], df_filtered: pd.DataFrame, df_all: pd.DataFrame):
    ...
    st.markdown(
        "Se muestran la tabla de sedes únicas, población involucrada (censo) y la oferta por servicio/especialidad."
    )
    ...
    # ----------------- Filtro periodo giros (mover antes del cálculo de oferta/detalle) -----------------
    giro_path = r"C:/REPS/data/giro.parquet"
    date_range = None
    giro_df_tmp = load_giro_parquet(giro_path)
    if giro_df_tmp is not None and not giro_df_tmp.empty:
        date_col_tmp = _find_column_like(giro_df_tmp, ["fecha", "fecha giro", "fecha_giro", "fecha_pago", "date", "periodo"])
        if date_col_tmp and date_col_tmp in giro_df_tmp.columns:
            try:
                giro_df_tmp['_parsed_date_tmp'] = pd.to_datetime(giro_df_tmp[date_col_tmp], errors='coerce')
            except Exception:
                giro_df_tmp['_parsed_date_tmp'] = pd.to_datetime(giro_df_tmp[date_col_tmp].astype(str), errors='coerce')
            min_date = giro_df_tmp['_parsed_date_tmp'].min()
            max_date = giro_df_tmp['_parsed_date_tmp'].max()
    ...
    # Apply filters to df_result (como antes)
    df_result = df.copy()
    ...
    # ---------------- Build identification table (one record per sede) ----------------
